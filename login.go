...
func (session *Session) LoginWithCode(accountName, password, code string) error {
	...
	//log.Printf("pollAuth: %+v", pollAuth)
...
func (session *Session) Login(accountName, password, sharedSecret string, timeOffset time.Duration) error {
	code, err := GenerateTwoFactorCode(sharedSecret, time.Now().Add(timeOffset).Unix())
	if err != nil {
		return err
	}

	return session.LoginWithCode(accountName, password, code)
}
